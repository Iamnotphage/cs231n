...
def rnn_backward(dh, cache):
    ...
    t = T - 1
    while t >= 0:
      dx_t, dh0_t, dWx_t, dWh_t, db_t = rnn_step_backward(dh[:, t, :] + dh0, cache[t])
...
def word_embedding_forward(x, W):
    ...
    #   for t in range(T):
    #     out[i, t] = W[x[i, t]]
    # W中有V个词汇(vocabulary) 一个vocabulary用D长度向量表示
    # X表示有N个句子，每个句子都有T个vocabulary
    # X[i, t]表示第i个句子的第t个单词(的index)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, prev_h, prev_c, next_c, Wx, Wh, i, f, o, g)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, next_c, Wx, Wh, i, f, o, g = cache
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      next_h, next_c, cache_t = lstm_step_forward(x[:, t, :], h0, prev_c, Wx, Wh, b)
...
def lstm_backward(dh, cache):
    ...
    while t >= 0:
      dx[:, t, :], dh0, dc0, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:, t, :] + dh0, dc0, cache[t])
