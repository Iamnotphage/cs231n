...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        denominator2 = 0
        for t in range(2 * N):
            denominator1 += np.exp(sim(z_k, out[t]) / tau)
            denominator2 += np.exp(sim(z_k_N, out[t]) / tau)

        denominator1 -= np.exp(sim(z_k, z_k) / tau)
        denominator2 -= np.exp(sim(z_k_N, z_k_N) / tau)

        l1 = -sim(z_k, z_k_N) / tau + np.log(denominator1)
        l2 = -sim(z_k_N, z_k) / tau + np.log(denominator2)

        total_loss = total_loss + l1 + l2
...
def sim_positive_pairs(out_left, out_right):
    ...
    out_left_norm = out_left / out_left.norm(dim=1, keepdim=True)
    out_right_norm = out_right / out_right.norm(dim=1, keepdim=True)
    pos_pairs = torch.sum(out_left_norm * out_right_norm, dim=1, keepdim=True)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    tot_spp = torch.cat((sim_pos_pairs, sim_pos_pairs))
    numerator = torch.exp(tot_spp / tau)
